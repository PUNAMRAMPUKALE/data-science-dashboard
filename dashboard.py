...
ratings_movies = ratings_movies.merge(movies[['movieId', 'title_clean', 'year']], on='movieId', how='left')
...
top_rated = ratings_mean_count[ratings_mean_count['rating_count'] >= 50].sort_values(by='rating_mean', ascending=False).head(10)

theme_color = 'Purples'

fig_top_rated = px.bar(top_rated.sort_values(by='rating_mean'), x='rating_mean', y='title', orientation='h',
                       labels={'rating_mean': 'Avg Rating', 'title': 'Movie'}, color='rating_mean',
                       color_continuous_scale=theme_color)
fig_top_rated.update_layout(title='Top Rated Movies', plot_bgcolor='white', paper_bgcolor='white', font_color='#333')

trend_data = ratings_movies.groupby('year')['rating'].mean().reset_index().dropna()
fig_trend = px.line(trend_data, x='year', y='rating', title='Avg Rating Over Years', markers=True,
                    color_discrete_sequence=['#7e57c2'])
fig_trend.update_layout(plot_bgcolor='white', paper_bgcolor='white', font_color='#333')

rating_dist_fig = px.histogram(ratings, x='rating', nbins=10, title='Rating Distribution',
                                color_discrete_sequence=['#7e57c2'])
rating_dist_fig.update_layout(plot_bgcolor='white', paper_bgcolor='white', font_color='#333')

most_rated_movies = ratings.groupby('movieId').count()['rating'].sort_values(ascending=False).head(10).reset_index()
most_rated_movies = most_rated_movies.merge(movies[['movieId', 'title']], on='movieId')
fig_most_rated = px.bar(most_rated_movies, x='rating', y='title', orientation='h', title='Most Rated Movies',
                        color_discrete_sequence=['#7e57c2'])
fig_most_rated.update_layout(plot_bgcolor='white', paper_bgcolor='white', font_color='#333')
...
fig_genre_heatmap.update_layout(title='Genres vs Year', plot_bgcolor='white', paper_bgcolor='white', font_color='#333')

# Pie chart by genre
genre_count = genre_expanded['genre'].value_counts().reset_index()
genre_count.columns = ['Genre', 'Count']
fig_genre_pie = px.pie(genre_count, values='Count', names='Genre', title='Movie Genre Distribution',
...
card = {
    ...
    "boxShadow": "0 4px 12px rgba(0,0,0,0.05)", "marginBottom": "20px"
}

app.layout = html.Div(style={"backgroundColor": "#f4f6fa", "padding": "30px", "fontFamily": "Segoe UI"}, children=[
    html.H2("🎬 Movie Analytics Dashboard", style={"color": "#5e2ca5", "marginBottom": "30px"}),

    html.Div([
        html.Div([
            html.Div(k, style={"fontSize": "16px", "color": "#8e44ad"}),
            html.Div(v, style={"fontSize": "28px", "fontWeight": "bold", "marginTop": "4px"})
    ...
    html.H2("🎭 Browse Movies by Genre", style={"color": "#5e2ca5", "marginTop": "40px", "marginBottom": "20px"}),
    ...
    html.Div([
        dcc.Dropdown(
            ...
            style={
                "width": "100%", "marginBottom": "20px", "color": "#5e2ca5",
                "backgroundColor": "#fdf9ff", "border": "1px solid #5e2ca5",
        ...
        dash_table.DataTable(
            ...
            page_size=10,
            page_action="custom",
            style_table={"overflowX": "auto", "border": "1px solid #ccc", "borderRadius": "6px"},
            style_cell={"padding": "10px", "backgroundColor": "#fff", "color": "#5e2ca5"},
            style_header={"backgroundColor": "#5e2ca5", "color": "#fff", "fontWeight": "bold"}
        )
    ], style={**card})
])
